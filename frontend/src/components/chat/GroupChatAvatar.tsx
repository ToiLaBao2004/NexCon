...
const GroupChatAvatar = ({ participants, type, groupAvatarUrl }: GroupChatAvatarProps) => {
    ...
    if (groupAvatarUrl) {
        return (
            <div
                className={cn(
                    "relative shrink-0",
                    isProfile ? "w-20 h-20" : isLarge ? "w-16 h-16" : isPeople ? "w-12 h-12" : isMedium ? "w-12 h-12" : "w-8 h-8"
                )}
            >
                <UserAvatar
    ...
    if (limit === 1) {
        ...
        childSizeClass = isProfile ? "!size-20 !text-2xl" : isLarge ? "!size-16 !text-base" : isPeople ? "!size-12 !text-base" : isMedium ? "!size-12 !text-base" : "!size-8 !text-sm";
    } else if (limit === 2) {
        positions = [
            "top-0 right-0 z-0",
            "bottom-0 left-0 z-10"
        ];
        childSizeClass = isProfile ? "!size-[52px] !text-base" : isLarge ? "!size-[42px] !text-sm" : isPeople ? "!size-[32px] !text-xs" : isMedium ? "!size-[32px] !text-sm" : "!size-[22px] !text-[10px]";
    } else if (limit === 3) {
        positions = [
        ...
        childSizeClass = isProfile ? "!size-[48px] !text-sm" : isLarge ? "!size-[38px] !text-xs" : isPeople ? "!size-[28px] !text-[11px]" : isMedium ? "!size-[28px] !text-xs" : "!size-[18px] !text-[9px]";
    } else {
        // limit >= 4
        positions = [
            "top-0 left-0 z-10",
        ...
        childSizeClass = isProfile ? "!size-[42px] !text-sm" : isLarge ? "!size-[34px] !text-xs" : isPeople ? "!size-[26px] !text-[10px]" : isMedium ? "!size-[26px] !text-xs" : "!size-[17px] !text-[8px]";
    }
