...
const GroupChatAvatar = ({ participants, type, groupAvatarUrl }: GroupChatAvatarProps) => {
    ...
    const isHeader = type === "chat";
    const isLarge = isSidebar || isHeader;
    ...
    if (groupAvatarUrl) {
        return (
            <div
                className={cn(
                    "relative shrink-0",
                    isProfile ? "w-20 h-20" : isLarge ? "w-16 h-16" : isPeople ? "h-14 w-14" : isMedium ? "w-12 h-12" : "w-8 h-8"
                )}
            >
                <UserAvatar
    ...
    if (limit === 1) {
        ...
        childSizeClass = isProfile ? "!size-[52px] !text-base" : isLarge ? "!size-[42px] !text-sm" : isPeople ? "!size-[38px] !text-sm" : isMedium ? "!size-[32px] !text-sm" : "!size-[22px] !text-[10px]";
    } else if (limit === 3) {
        positions = [
        ...
        childSizeClass = isProfile ? "!size-[48px] !text-sm" : isLarge ? "!size-[38px] !text-xs" : isPeople ? "!size-[34px] !text-xs" : isMedium ? "!size-[28px] !text-xs" : "!size-[18px] !text-[9px]";
    } else {
        // limit >= 4
        positions = [
            "top-0 left-0 z-10",
        ...
        childSizeClass = isProfile ? "!size-[42px] !text-sm" : isLarge ? "!size-[34px] !text-xs" : isPeople ? "!size-[31px] !text-[11px]" : isMedium ? "!size-[26px] !text-xs" : "!size-[17px] !text-[8px]";
    }
